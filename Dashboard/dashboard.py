...
# Define a bar chart generation function
...
app_2.layout = html.Div([
    ...
    # the second line: Date picker and basic information
    html.Div([
        # basic info
        html.Div([
            html.Div([
                html.H6('Total Viewers', style={'textAlign': 'center', 'color': 'white', 'font-family': 'Verdana'}),
            ...
            # Date picker and current day data
            html.Div([
                ...
                html.H6('Viewers of Selected Day', style={'textAlign': 'center', 'color': 'white', 'font-family': 'Verdana', 'marginTop': '20px'}),
                html.P(id='total-viewers-selected', children=f"{today_visits:,.0f}", 
                       style={'textAlign': 'center', 'color': 'orange', 'fontSize': 32}),
                html.H6('Avg Completion Rate', style={'textAlign': 'center', 'color': 'white', 'font-family': 'Verdana', 'marginTop': '20px'}),
        ...
        html.Div([
            dcc.Tabs(id='tabs', value='gender-tab', children=[
                dcc.Tab(label='Gender Completion', value='gender-tab', children=[
                    dcc.Graph(id='gender-completion-chart', figure=create_bar_chart(gender_completion_counts, "Gender Completion Rate Distribution", "Gender", color_palette))
                ]),
                dcc.Tab(label='Age Completion', value='age-tab', children=[
                    dcc.Graph(id='age-completion-chart', figure=create_bar_chart(age_completion_counts, "Age Group Completion Rate Distribution", "Age Group", color_palette))
                ]),
                dcc.Tab(label='Ethnicity Completion', value='ethnicity-tab', children=[
                    dcc.Graph(id='ethnicity-completion-chart', figure=create_bar_chart(ethnicity_completion_counts, "Ethnicity Completion Rate Distribution", "Ethnicity", color_palette))
                ]),
        ...
        html.Div([
            dcc.Graph(id='pie-chart', figure={
                'data': [go.Pie(labels=overall_completion_counts.index, values=overall_completion_counts.values, hole=0.3, marker={'colors': color_palette}, textinfo='percent', textfont={'size': 16}, textposition='auto')],
                'layout': go.Layout(
    ...
    html.Div([
        html.Div([
            ...
            dcc.Dropdown(id='ad-dropdown', options=[{'label': ad_id, 'value': ad_id} for ad_id in weekly_ad_avg_completion['ad_id'].unique()], value=weekly_ad_avg_completion['ad_id'].unique()[0], style={'background-color': '#1f2c56', 'color': 'white', 'optionHeight': 30}),
            html.P('Select Time Granularity:', className='fix_label', style={'color': 'white'}),
            dcc.RadioItems(id='time-granularity', options=[{'label': 'Daily', 'value': 'daily'}, {'label': 'Weekly', 'value': 'weekly'}, {'label': 'Monthly', 'value': 'monthly'}], value='daily', labelStyle={'display': 'inline-block', 'color': 'white', 'margin-right': '10px', 'font-family': 'Verdana'}),
            dcc.Graph(id='line-chart', config={'scrollZoom': True, 'displayModeBar': True, 'modeBarButtonsToRemove': ['lasso2d', 'select2d']}),
...
# Update date-related callback functions
@app_2.callback(
...
def update_all(selected_date):
    ...
    daily_overall_completion_counts = daily_data['completion_level'].value_counts().reindex(['0-20%', '20-40%', '40-60%', '60-80%', '80-100%'], fill_value=0)
    ...
    pie_chart = {
        'data': [go.Pie(labels=daily_overall_completion_counts.index, values=daily_overall_completion_counts.values, hole=0.3, marker={'colors': color_palette}, textinfo='percent', textfont={'size': 16}, textposition='auto')],
        'layout': go.Layout(title='Overall Completion Rate Distribution', paper_bgcolor='#1f2c56', plot_bgcolor='#1f2c56', font={'color': 'white'}, margin=dict(l=0, r=0, t=50, b=100), legend=dict(orientation="h", yanchor="bottom", y=-0.3, xanchor="center", x=0.5), autosize=True)
    }

    return (f"{daily_visits:,.0f}", f"{daily_avg_completion_rate}%", gender_chart, age_chart, ethnicity_chart, pie_chart)

# Generate timeline and sliders
@app_2.callback(
    [Output('time-axis-store', 'data'), Output('slider-container', 'children')],
    [Input('ad-dropdown', 'value'), Input('time-granularity', 'value')]
)
def generate_time_axis(ad_id, time_granularity):
    ...
    if time_granularity == 'daily':
        ...
        weekly_data = weekly_ad_avg_completion[weekly_ad_avg_completion['ad_id'] == ad_id]
        ...
        monthly_data = monthly_ad_avg_completion[monthly_ad_avg_completion['ad_id'] == ad_id]
    ...
    slider = dcc.Slider(id='time-slider', min=0, max=len(time_points) - 1, step=1, value=len(time_points) - 1, marks={i: time_points[i] for i in range(0, len(time_points), max(1, len(time_points) // 5))}, tooltip={"placement": "bottom", "always_visible": True})
    return time_points, slider

# update the line chart
@app_2.callback(
    Output('line-chart', 'figure'),
    [Input('ad-dropdown', 'value'), Input('time-granularity', 'value'), Input('time-slider', 'value'), Input('time-axis-store', 'data')]
)
def update_line_chart(ad_id, time_granularity, slider_value, time_points):
    ...
    if time_granularity == 'daily':
        ...
        filtered_data = data[(data['ad_id'] == ad_id) & (data['visit_date'].isin(all_dates))]
        agg_data = filtered_data.groupby('visit_date')['completion_rate'].mean().reindex(all_dates).fillna(0).reset_index()
        agg_data.columns = ['visit_date', 'completion_rate']
        x_data = agg_data['visit_date']
        y_data = agg_data['completion_rate'] * 100  # 转换为百分比
        title_suffix = 'Daily Completion Rate Trend'

    elif time_granularity == 'weekly':
        weekly_data = weekly_ad_avg_completion[(weekly_ad_avg_completion['ad_id'] == ad_id) & (weekly_ad_avg_completion['year_week'].isin(display_time_points))]
        all_weeks = pd.Series(display_time_points)
        agg_data = weekly_data.groupby('year_week')['completion_rate'].mean().reindex(all_weeks).fillna(0).reset_index()
        ...
        monthly_data = monthly_ad_avg_completion[(monthly_ad_avg_completion['ad_id'] == ad_id) & (monthly_ad_avg_completion['month'].isin(display_time_points))]
        all_months = pd.Series(display_time_points)
        agg_data = monthly_data.groupby('month')['completion_rate'].mean().reindex(all_months).fillna(0).reset_index()
        agg_data.columns = ['month', 'completion_rate']
        x_data = agg_data['month']
        y_data = agg_data['completion_rate'] * 100
        title_suffix = 'Monthly Completion Rate Trend'

    fig.add_trace(go.Scatter(x=x_data, y=y_data, mode='lines+markers', line={'color': '#FFA500', 'width': 3}, marker={'size': 10, 'color': 'orange'}, name='Completion Rate'))
    fig.update_layout(
...
if __name__ == '__main__':
    ...
    <html>
        <head>
            ...
            <style>
                ...
                .DateInput_input { background-color: #1f2c56 !important; color: white !important; font-size: 16px !important; width: 100% !important; font-family: Verdana !important; }
    ...
    '''
    app_2.run_server(debug=True, dev_tools_ui=False, dev_tools_props_check=False, port=5002)