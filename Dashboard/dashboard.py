...
app_2.layout = html.Div([
    ...
    html.Div([
        html.Div([
            ...
            html.Div([  # Wrap label and buttons in a centered container
                html.P('Select Time Granularity:', className='fix_label', style={'color': 'white', 'textAlign': 'center', 'marginBottom': '10px'}),
                html.Div([
                ...
                ], style={'display': 'flex', 'justifyContent': 'center'}),  # Center the buttons
            ], style={'display': 'flex', 'flexDirection': 'column', 'alignItems': 'center'}),  # Center the entire block (label + buttons)
...
# Callback for updating date-related components
@app_2.callback(
    [
...
# Callback for updating time granularity store based on button clicks
@app_2.callback(
...
# Callback for generating timeline and sliders
@app_2.callback(
...
def update_line_chart(ad_id, time_granularity, slider_value, time_points):
    ...
    if time_granularity == 'daily':
        ...
        weekly_data = weekly_ad_avg_completion[
        ...
        monthly_data = monthly_ad_avg_completion[
...
# Run the app with custom index string
