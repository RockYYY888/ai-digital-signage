...
def generate_response(model, tokenizer, messages, max_new_tokens=80):
...
def get_relevant_background(product_name):
    """根据产品名称获取相关背景信息。可以扩展为更复杂的逻辑。"""
    # 假设简单的匹配机制，根据产品名称选择背景信息
...
def generate_input_text_with_context(product_name, race, age_range, gender, tone, context):
...
def generate_ad_with_context(model, tokenizer, input_str, tone='Natural', max_new_tokens=60, verbose=False):
    ...
    input_text = generate_input_text_with_context(product_name, race, age_range, gender, tone, context)
...
if __name__ == "__main__":
    ...
    input_str = "laundrydetergent, white, 50+, female"
    ad_text = generate_ad_with_context(model, tokenizer, input_str, tone='Natural', verbose=False)
